...
public class autoCompleteStmd extends BaseAction{
	...
	public String execute() {
		...
		tmp.addAll(df.sqlGet("SELECT IFNULL(css.name,'一般生')as status, s.student_no, s.student_name, c.ClassName FROM Gstmd s LEFT OUTER JOIN CODE_STMD_STATUS css ON s.occur_status=css.id, Class c WHERE " +
				"s.depart_class=c.ClassNo AND (s.student_no LIKE '"+request.getParameter("nameno")+"%' || s.student_name LIKE '%"+request.getParameter("nameno")+"%') LIMIT 10"));
