...
(function() {
	...
	function b(b) {
		if (o) {
			...
			dhtmlx.message.keyboard && ((c == 13 || c == 32) && a(o, !0), c == 27 && a(o, !1));
	...
	function e(a) {
		if (!l.area) l.area = document.createElement("DIV"), l.area.className = "dhtmlx_message_area", l.area.style[l.position] = "5px", document.body.appendChild(l.area);
		l.hide(a.id);
	...
	var l = dhtmlx.message = function(a, b, c, d) {
...
scheduler._click = {
	...
	dhx_cal_today_button: function() {
		scheduler.callEvent("onBeforeTodayDisplayed", []) && scheduler.setCurrentView(new Date)
	},
...
scheduler.addEventNow = function(a, b, c) {
	...
	a || (a = d.start_date || Math.round((new Date).valueOf() / e) * e);
	var f = new Date(a);
	if (!b) {
...
scheduler._mouse_coords = function(a) {
	...
	if (this._table_view) {
		...
		b.x = 0
	} else b.x = Math.min(this._cols.length - 1, Math.max(0, Math.ceil(b.x / this._cols[0]) - 1)), b.y = Math.max(0, Math.ceil(b.y * 60 / (this.config.time_step * this.config.hour_size_px)) - 1) + this.config.first_hour * (60 / this.config.time_step);
	return b
};
...
scheduler._on_mouse_move = function(a) {
	if (this._drag_mode) {
		...
		if (!this._drag_pos || b.force_redraw || this._drag_pos.x != b.x || this._drag_pos.y != b.y) {
			...
			if (this._drag_mode == "move") c = this._min_date.valueOf() + (b.y * this.config.time_step + b.x * 1440) * 6E4, !b.custom && this._table_view && (c += this.date.time_part(h.start_date) * 1E3), c = this._correct_shift(c), d = h.end_date.valueOf() - (h.start_date.valueOf() - c);
			else {
				c = h.start_date.valueOf();
				d = h.end_date.valueOf();
				if (this._table_view) {
					var k = this._min_date.valueOf() + b.y * this.config.time_step * 6E4 + (b.custom ? 0 : 864E5);
					this._mode == "month" && (k = this._correct_shift(k, !1));
					b.resize_from_start ? c = k : d = k
				...
				if (this._drag_mode == "new-size") if (d <= this._drag_start) {
					var i = b.shift || (this._table_view && !b.custom ? 864E5 : 0);
					c = d - (b.shift ? 0 : i);
					d = this._drag_start + (i || this.config.time_step * 6E4)
				} else c = this._drag_start;
				else d <= c && (d = c + this.config.time_step * 6E4)
			}
			var j = new Date(d - 1),
				m = new Date(c);
			if (this._table_view || j.getDate() == m.getDate() && j.getHours() < this.config.last_hour || scheduler._allow_dnd) if (h.start_date = m, h.end_date = new Date(d), this.config.update_render) {
				var o = scheduler._els.dhx_cal_data[0].scrollTop;
				this.update_view();
				scheduler._els.dhx_cal_data[0].scrollTop = o
		...
		var l = this._locate_event(a.target || a.srcElement);
		this.callEvent("onMouseMove", [l, a])
	}
};
	...
	if (this.templates[this._mode + "_date"]) {
		...
		for (var l = 0; l < m; l++) {
			...
			if (!this._table_view) {
				...
				this.set_xy(p, this._cols[l] - 1, c.hour_size_px * (c.last_hour - c.first_hour), f + this.xy.scale_width + 1, 0);
				b.appendChild(p);
		...
		if (this._table_view) this._reset_month_scale(b, h, k);
			...
			var n = "dhx_multi_day";
			this._els[n] && (this._els[n][0].parentNode.removeChild(this._els[n][0]), this._els[n] = null);
			var r = this._els.dhx_cal_navline[0],
				s = r.offsetHeight + this._els.dhx_cal_header[0].offsetHeight + 1,
				u = document.createElement("DIV");
			u.className = n;
			u.style.visibility = "hidden";
			this.set_xy(u, this._colsS[this._colsS.col_length - 1] + this.xy.scroll_width, 0, 0, s);
			b.parentNode.insertBefore(u, b);
			var t = u.cloneNode(!0);
			t.className = n + "_icon";
			t.style.visibility = "hidden";
			this.set_xy(t, this.xy.scale_width, 0, 0, s);
			u.appendChild(t);
			this._els[n] = [u, t];
			this._els[n][0].onclick = this._click.dhx_cal_data
		}
	}
};
...
scheduler._reset_month_scale = function(a, b, c) {
	...
	var m = 0;
	this._min_date = c;
	for (var o = "<table cellpadding='0' cellspacing='0'>", l = [], i = 0; i < f; i++) {
		o += "<tr>";
		for (var p = 0; p < 7; p++) {
			o += "<td";
			var q = "";
			c < b ? q = "dhx_before" : c >= d ? q = "dhx_after" : c.valueOf() == e.valueOf() && (q = "dhx_now");
			o += " class='" + q + " " + this.templates.month_date_class(c, e) + "' >";
			var n = "dhx_month_body",
				r = "dhx_month_head";
			p === 0 && this.config.left_border && (n += " dhx_month_body_border", r += " dhx_month_head_border");
			o += "<div class='" + r + "'>" + this.templates.month_day(c) + "</div>";
			o += "<div class='" + n + "' " + g[p] + "></div></td>";
			l.push(c);
	...
	a.innerHTML = o;
	this._scales = {};
	for (var s = a.getElementsByTagName("div"), i = 0; i < l.length; i++) {
		var u = s[i * 2 + 1],
			t = l[i];
		this._scales[+t] = u
	}
	for (i = 0; i < l.length; i++) t = l[i], this.callEvent("onScaleAdd", [this._scales[+t], t]);
	return c
};
...
scheduler.locale = {
	date: {
		month_full: "1月,2月,3月,4月,5月,6月,7月,8月,9月,10月,11月,12月".split(","),
		month_short: "1月,2月,3月,4月,5月,6月,7月,8月,9月,10月,11月,12月".split(","),
		day_full: "週日,週一,週二,週三,週四,週五,週六".split(","),
		day_short: "週日,週一,週二,週三,週四,週五,週六".split(",")
	},
	labels: {
		...
		icon_delete: "刪除",
		confirm_closing: "",
		confirm_deleting: "確定刪除?",
		section_description: "說明",
		section_time: "時間",
		full_day: "全天",
		confirm_recurring: "確定變更計劃?",
		section_recurring: "重覆循環",
		button_recurring: "開啟",
		button_recurring_open: "關閉",
		button_edit_series: "變更計劃週期",
		button_edit_occurrence: "變更計劃",
		agenda_tab: "Agenda",
		date: "日期",
		description: "說明",
		year_tab: "Year",
...
scheduler.init_templates = function() {
	...
	d(scheduler.templates, {
		...
		week_date: function(a, b) {
			return scheduler.templates.day_date(a) + " 至 " + scheduler.templates.day_date(scheduler.date.add(b, -1, "day"))
		},
	...
	if (this._table_view) if (b) this._colsS.heights = d;
		...
		if (g.rows) {
			...
			var o = g.parentNode.childNodes,
				...
				p = l,
				q = l + "px";
			this.config.multi_day_height_limit && (p = Math.min(l, this.config.multi_day_height_limit), q = p + "px");
			f.style.top = this._els.dhx_cal_navline[0].offsetHeight + this._els.dhx_cal_header[0].offsetHeight + p + "px";
			...
			var r = this._els.dhx_multi_day[1];
			r.style.height = q;
			r.style.visibility = e[0] == -1 ? "hidden" : "visible";
			r.className = e[0] ? "dhx_multi_day_icon" : "dhx_multi_day_icon_small";
			this._dy_shift = (e[0] + 1) * c;
			e[0] = 0;
			if (p != l) f.style.top = parseInt(f.style.top) + 2 + "px", n.style.overflowY = "auto", n.style.width = parseInt(n.style.width) - 2 + "px", r.style.position = "fixed", r.style.top = "", r.style.left = ""
		}
	}
	return a
};
	...
	for (var e = 0; e < a.length; e++) {
		...
		if (!b) {
			...
			for (var j = c[f._sday]; j.length;) {
				...
				if (o.valueOf() <= f.start_date.valueOf()) j.splice(j.length - 1, 1);
				else break
			}
			for (var l = !1, p = 0; p < j.length; p++) if (m = j[p], o = this._get_event_mapped_end_date(m), o.valueOf() <= f.start_date.valueOf()) {
				l = !0;
				f._sorder = m._sorder;
				j.splice(p, 1);
				f._inner = !0;
			...
			if (!l) if (j.length) if (j.length <= j[j.length - 1]._sorder) {
				if (j[j.length - 1]._sorder) for (p = 0; p < j.length; p++) {
					for (var q = !1, n = 0; n < j.length; n++) if (j[n]._sorder == p) {
						q = !0;
						break
					}
					if (!q) {
				...
				q = j[0]._sorder;
				for (p = 1; p < j.length; p++) if (j[p]._sorder > q) q = j[p]._sorder;
				f._sorder = q + 1;
				f._inner = !1
			} else f._sorder = 0;
			j.push(f);
...
scheduler._pre_render_events_table = function(a, b) {
	...
	for (var c = [], d = [
		...
		if (l > this._max_date) m.last_chunk = !1, l = this._max_date;
		var p = this.locate_holder_day(o, !1, i);
		i._sday = p % g;
		var q = this.locate_holder_day(l, !0, i) || g;
		i._eday = q % g || g;
		i._length = q - p;
		i._sweek = Math.floor((this._correct_shift(o.valueOf(), 1) - this._min_date.valueOf()) / (864E5 * g));
		var n = d[i._sweek],
			r;
		for (r = 0; r < n.length; r++) if (n[r]._eday <= i._sday) break;
		if (!i._sorder || !b) i._sorder = r;
		if (i._sday + i._length <= g) f = null, c.push(i), n[r] = i, e[i._sweek] = n.length - 1, i._first_chunk = m.first_chunk, i._last_chunk = m.last_chunk;
		else {
			...
			s._sweek = i._sweek;
			s._sorder = i._sorder;
			s.end_date = this.date.add(o, s._length, "day");
			if (s._first_chunk = m.first_chunk) m.first_chunk = !1;
			c.push(s);
			n[r] = s;
			f = s.end_date;
			e[i._sweek] = n.length - 1;
			k--
		}
	}
	return c
	...
	if (!(a._sday < 0)) {
		...
		if (d) {
			...
			if (this.config.cascade_event_display) var o = this.config.cascade_event_count,
				l = this.config.cascade_event_margin,
				m = h % o * l,
				p = a._inner ? (g - h - 1) % o * l / 2 : 0,
				j = Math.floor(d.clientWidth - c - m - p);
			var q = this._render_v_bar(a.id, c + m, k, j, i, a._text_style, scheduler.templates.event_header(a.start_date, a.end_date, a), scheduler.templates.event_text(a.start_date, a.end_date, a));
			this._rendered.push(q);
			d.appendChild(q);
			m = m + parseInt(d.style.left, 10) + c;
			if (this._edit_id == a.id) {
				q.style.zIndex = 1;
				j = Math.max(j - 4, scheduler.xy.editor_width);
				q = document.createElement("DIV");
				q.setAttribute("event_id", a.id);
				this.set_xy(q, j, i - 20, m, k + 14);
				q.className = "dhx_cal_editor";
				var n = document.createElement("DIV");
				this.set_xy(n, j - 6, i - 26);
				n.style.cssText += ";margin:2px 2px 2px 2px;overflow:hidden;";
				q.appendChild(n);
				this._els.dhx_cal_data[0].appendChild(q);
				this._rendered.push(q);
				n.innerHTML = "<textarea class='dhx_cal_editor'>" + a.text + "</textarea>";
				if (this._quirks7) n.firstChild.style.height = i - 12 + "px";
				this._editor = n.firstChild;
			...
			if (this.xy.menu_width !== 0 && this._select_id == a.id) {
				...
				var x = this._render_v_bar(a.id, m - b + 1, k, b, r.length * 20 + 26 - 2, "", "<div style='" + u + "" + t + "' class='dhx_menu_head'></div>", s, !0);
				x.style.left = m - b + 1;
...
scheduler._render_v_bar = function(a, b, c, d, e, f, g, h, k) {
	...
	var l = j.color ? "background:" + j.color + ";" : "",
		p = j.textColor ? "color:" + j.textColor + ";" : "",
		q = '<div event_id="' + a + '" class="' + m + '" style="position:absolute; top:' + c + "px; left:" + b + "px; width:" + (d - 4) + "px; height:" + e + "px;" + (f || "") + '"></div>';
	i.innerHTML = q;
	var n = i.cloneNode(!0).firstChild;
	if (!scheduler.renderEvent || k || !scheduler.renderEvent(n, j)) {
		var n = i.firstChild,
			r = '<div class="dhx_event_move dhx_header" style=" width:' + (d - 6) + "px;" + l + '" >&nbsp;</div>';
		r += '<div class="dhx_event_move dhx_title" style="' + l + "" + p + '">' + g + "</div>";
		r += '<div class="dhx_body" style=" width:' + (d - (this._quirks ? 4 : 14)) + "px; height:" + (e - (this._quirks ? 20 : 30) + 1) + "px;" + l + "" + p + '">' + h + "</div>";
		var s = "dhx_event_resize dhx_footer";
		k && (s = "dhx_resize_denied " + s);
		r += '<div class="' + s + '" style=" width:' + (d - 8) + "px;" + (k ? " margin-top:-1px;" : "") + "" + l + "" + p + '" ></div>';
		n.innerHTML = r
	}
	return n
};
...
scheduler.json.parse = function(a) {
	if (typeof a == "string") scheduler._temp = eval("(" + a + ")"), a = scheduler._temp ? scheduler._temp.data || scheduler._temp : [];
	...
	for (var j = [], m = 0; m < a.length; m++) {
		var o = a[m];
		scheduler._init_event(o);
		j.push(o)
	}
	return j
};
...
scheduler._magic_parser = function(a) {
	...
	for (var e = a.doXPath("//coll_options"), f = 0; f < e.length; f++) {
		...
		if (h) {
			...
			for (var k = a.doXPath(".//item", e[f]), i = 0; i < k.length; i++) {
				for (var j = k[i], m = j.attributes, o = {
	...
	for (var q = a.doXPath("//userdata"), f = 0; f < q.length; f++) {
		var n = this.xmlNodeToJSON(q[f]);
		this._userdata[n.name] = n.text
	}
	var r = [];
	b = a.doXPath("//event");
	for (f = 0; f < b.length; f++) {
		var s = r[f] = this.xmlNodeToJSON(b[f]);
...
scheduler.form_blocks = {
	...
	time: {
		render: function(a) {
			...
			if (!a.time_format) a.time_format = ["%Y", "%m", "%d", "%H:%i"];
			a._time_format_order = {};
			var b = a.time_format,
				c = scheduler.config,
				d = this.date.date_part(new Date),
				e = 1440,
				f = 0;
			...
			for (var g = "", h = 0; h < b.length; h++) {
				...
				case "%H:%i":
					...
					for (a._time_values = []; j < e;) {
						var o = this.templates.time_picker(d);
						g += "<option value='" + j + "'>" + o + "</option>";
						a._time_values.push(j);
						d.setTime(d.valueOf() + this.config.time_step * 6E4);
						var l = d.getDate() != m ? 1 : 0,
							j = l * 1440 + d.getHours() * 60 + d.getMinutes()
					}
					g += "</select>"
				}
			}
